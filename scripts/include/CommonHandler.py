...
# Let cfg_dict be the global_variable space in execfile method performing.
        cfg_dict = dict()
        cfg_abspath = os.path.join( self.FILENAME_CONFIG_DIR, self.FILENAME_DEFAULT_CONFIG)
        execfile(cfg_abspath, cfg_dict)

# Let local setting overwrite default setting
        if self.FILENAME_MY_CONFIG :
            my_cfg_dict = dict()
            mycfg_abspath = os.path.join( self.FILENAME_CONFIG_DIR, self.FILENAME_MY_CONFIG)
